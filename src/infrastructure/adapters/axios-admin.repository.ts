...
function serviceFormData(
  ...
  Object.entries(input).forEach(([key, value]) => {
    appendFormValue(
      formData,
      key,
      value as string | number | boolean | File | null,
    )
  })
...
export class AxiosAdminRepository
  ...
  async getHotelMedia(
    ...
    const page = await getPage<AdminHotelMedia>(
      apiConfig.endpoints.catalog.hotelMedia,
      {
        hotel: hotelId,
        pageSize: 100,
        ordering: 'orden',
  ...
  async createHotelMedia(
    input: SaveHotelMediaInput,
  ): Promise<AdminHotelMedia> {
    const { data } = await apiClient.post<AdminHotelMedia>(
      apiConfig.endpoints.catalog.hotelMedia,
      hotelFormData(input),
    )
  ...
  async updateHotelMedia(
    id: number,
    input: Partial<Omit<SaveHotelMediaInput, 'archivo'>>,
  ): Promise<AdminHotelMedia> {
    const { data } = await apiClient.patch<AdminHotelMedia>(
      `${apiConfig.endpoints.catalog.hotelMedia}${id}/`,
      hotelFormData(input),
    )
  ...
  async deleteHotelMedia(id: number): Promise<void> {
    await apiClient.delete(
      `${apiConfig.endpoints.catalog.hotelMedia}${id}/`,
    )
  }
