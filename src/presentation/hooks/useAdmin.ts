...
export const adminKeys = {
  ...
  hotelMedia: ['admin', 'hotel-media'] as const,
...
export function useHotelMediaQuery(hotelId: number | null) {
  return useQuery({
    queryKey: [...adminKeys.hotelMedia, hotelId],
    queryFn: () => adminUseCase.getHotelMedia(hotelId as number),
    enabled: hotelId !== null && hotelId > 0,
  })
}

export function useCreateHotelMediaMutation() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (input: SaveHotelMediaInput) => adminUseCase.createHotelMedia(input),
    onSuccess: async (_, input) => {
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: [...adminKeys.hotelMedia, input.hotel],
        }),
        queryClient.invalidateQueries({
...
export function useUpdateHotelMediaMutation() {
  const queryClient = useQueryClient()
  return useMutation({
    ...
    }) => adminUseCase.updateHotelMedia(variables.id, variables.input),
    onSuccess: async (_, variables) => {
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: [...adminKeys.hotelMedia, variables.hotelId],
        }),
        queryClient.invalidateQueries({
          queryKey: ['hotel-media'],
        }),
...
export function useDeleteHotelMediaMutation() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (variables: { id: number; hotelId: number }) =>
      adminUseCase.deleteHotelMedia(variables.id),
    onSuccess: async (_, variables) => {
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: [...adminKeys.hotelMedia, variables.hotelId],
